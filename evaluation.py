...
def acc_update(model, curr_image_idx, image_labels, spike_counts_per_image, accuracies):
    # Get image labels for current interval
    image_labels_curr_interval = image_labels[curr_image_idx - model.args.acc_update_interval:curr_image_idx]
...
def get_accuracy(model, spike_counts_per_image, image_labels):
    if not model.args.test_phase:
        _assign_neurons_to_labels(model, spike_counts_per_image, image_labels)

    predictions_per_image = _get_predictions(model, spike_counts_per_image)
...
def _assign_neurons_to_labels(model, spike_counts_per_image: ty.List[ty.List[int]], image_labels: np.ndarray) -> None:
    ...
    np.save(f'{model.run_path}/assignments_from_training.npy', assigned_labels)


def _get_predictions_for_current_image(spike_counts_current_image: ty.List[int],
                                       assignments_from_training: np.ndarray) -> ty.List[int]:
    predictions = []
    for label in range(10):
...
def _get_predictions(model, spike_counts_per_image: ty.List[ty.List[int]]) -> ty.List[ty.List[int]]:
    assignments_from_training = np.load(f'{model.run_path}/assignments_from_training.npy')
    test_image_count = len(spike_counts_per_image)
    predictions_per_image = []
    for image_idx in range(test_image_count):
        predictions_current_image = _get_predictions_for_current_image(spike_counts_per_image[image_idx],
                                                                       assignments_from_training)
